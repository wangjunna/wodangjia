...
import com.example.wodangjialayout.R.string;
import com.example.wodangjialayout.R.style;
...
//			showTypeDialog();
	...
	private void uploadGoods() {
		...
		final CustomProgressDialog pd =new CustomProgressDialog(AddGoodsActivity.this,"正在上传商品信息,请稍等！",R.anim.frame);
		pd.show();
		RequestParams params = new RequestParams();
		params.addBodyParameter(Config.KEY_ACTION,Config.ACTION_ADD_GOOD);
...
//		params.addBodyParameter(Config.KEY_ACTION, Config.ACTION_ADD_GOOD);
		// params.addBodyParameter(Config.KEY_GOODS_TITLE,
		// Config.ACTION_ADD_GOOD);
		...
		App.httpclient.send(HttpRequest.HttpMethod.POST, Config.UPLOAD_URL,
				params, new RequestCallBack<String>() {
					...
					public void onSuccess(ResponseInfo<String> responseInfo) {
						...
						StringUtils.showToast(AddGoodsActivity.this, "商品上传成功！");
					...
					public void onFailure(HttpException error, String msg) {
						// 上传失败
						pd.dismiss();
						StringUtils.showToast(AddGoodsActivity.this, "商品上传失败，请检查网络！");
	...
	private void openCamera() {
		...
		intent.putExtra(MediaStore.EXTRA_OUTPUT, Uri.fromFile(new File(
				AddGoodsActivity.this.getExternalCacheDir(), tempImgName)));
		...
		if (index == -1) {
			...
			imageView.setOnClickListener(new OnClickListener() {
				...
				public void onClick(final View v) {
					new AlertDialog.Builder(AddGoodsActivity.this)
	...
	void removeImageView(View v) {
		...
		StringUtils.showToast(AddGoodsActivity.this,
	...
	void showTypeDialog() {
		...
		if(typeList.size()>0){
			dialog=new ActionSheetDialog(AddGoodsActivity.this)
			.builder()
			.setTitle("请选择商品类别")
			.setCancelable(true)
			...
			for(int i=0;i<typeList.size();i++){
				...
				dialog.addSheetItem(title, SheetItemColor.Gray, new OnSheetItemClickListener() {
					@Override
	...
	void getType(){
		...
		App.httpclient.send(HttpMethod.POST, Config.API_URL, params, new RequestCallBack<String>() {
			...
			public void onFailure(HttpException arg0, String arg1) {
				StringUtils.showToast(AddGoodsActivity.this, Config.ERROR_GET_TYPES);
			}
			...
			public void onSuccess(ResponseInfo<String> arg0) {
				...
				if (status==Config.RESULT_STATUS_SUCCESS) {
					Gson gson= new Gson();
					Type type=new TypeToken<List<Goods_Type>>() {}.getType(); 
					typeList=gson.fromJson(JsonUtils.getJsonArray(arg0.result, "type_list").toString(), type);
